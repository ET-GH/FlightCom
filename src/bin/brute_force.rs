//! THIS BINARY IS FOR A REAL FLIGHT (IN THEORY BUT IT MIGHT NOT FUCKING WORK IDK MAN)
...
use mcu::{motor, sensors::imu};

/// Seconds used to establish pad acceleration baseline.
const CALIBRATION_TIME_S: u64 = 3;
...
async fn main(spawner: Spawner) {
    ...
    // Pad calibration. Ideally we use Welford online variance/mean
    // but I ain't got time for that shit rn.
    let deadline = Instant::now() + Duration::from_secs(CALIBRATION_TIME_S);
    let mut calibration_samples: u32 = 0;
    let mut magnitude_sum_g = 0.0;
    let mut axial_sum_g = 0.0;
    while Instant::now() < deadline {
        let data = imu::DATA.wait().await;
        let magnitude_g = data.magnitude();
        // On the launch pad, with the rocket vertical, this axis reads
        // approximately +1 g. Positive motor thrust should increase this value.
        let axial_g = data.accel_g.y;
    ...
    if calibration_samples == 0 {
        defmt::panic!("imu calibration received no samples what the fuck");
    }
    ...
    loop {
        let data = imu::DATA.wait().await;
        if rocket
            .update(data.magnitude(), data.accel_g.y, data.dt_s)
            .await
        {
            break;
        }
...
impl RocketStateMachine {
    ...
    async fn update(&mut self, magnitude_g: f32, axial_g: f32, dt_s: f32) -> bool {
